...
// WithPoolLimits returns an option to configure basic pool limits.
func WithPoolLimits(minConns, maxConns int32) func(*pgxpool.Config) {
...
// WithPoolTimings returns an option to configure pool timings.
func WithPoolTimings(maxLifetime, maxIdle, healthCheck time.Duration) func(*pgxpool.Config) {
    return func(cfg *pgxpool.Config) {
        if maxLifetime > 0 {
